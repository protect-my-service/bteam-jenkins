...
- `.env` 대신 SSM Parameter Store → systemd `EnvironmentFile`로 주입 (상세 runbook은 별도 PR)
- **EC2에 `docker-compose.local.yml`을 배포하지 마세요** (rsync/Ansible에서 exclude)
- `JENKINS_URL`은 `prod.yml`에서 `${JENKINS_URL:?}` 검사로 누락 시 기동 실패
