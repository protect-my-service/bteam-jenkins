...
AGENT_SG=$(aws ec2 create-security-group --group-name jenkins-agent \
  --description "Jenkins agent" --vpc-id $VPC_ID --query GroupId --output text)
# 50000: agent → controller (controller가 받는 쪽 인바운드)
aws ec2 authorize-security-group-ingress --group-id $CTRL_SG \
  --protocol tcp --port 50000 --source-group $AGENT_SG
...
aws ssm put-parameter --name /jenkins/JENKINS_URL \
  --type String --value 'https://jenkins.example.com/'
...
#!/usr/bin/env bash
export JENKINS_REPO_URL=https://github.com/<org>/bteam-jenkins.git
...
exec bash <(curl -fsSL https://raw.githubusercontent.com/<org>/bteam-jenkins/main/scripts/user-data-controller.sh)
EOF
...
exec bash <(curl -fsSL https://raw.githubusercontent.com/<org>/bteam-jenkins/main/scripts/user-data-agent.sh)
EOF
...
CERT_ARN=$(aws acm list-certificates --query \
  "CertificateSummaryList[?DomainName=='jenkins.example.com'].CertificateArn" --output text)
...
aws route53 change-resource-record-sets --hosted-zone-id $ZONE_ID \
  --change-batch "{\"Changes\":[{\"Action\":\"UPSERT\",\"ResourceRecordSet\":{
    \"Name\":\"jenkins.example.com\",\"Type\":\"A\",
...
ASG로 컨트롤러가 InService가 되면 (`https://jenkins.example.com/` 접속 가능):
...
| # | 절차 | 기대 |
|---|---|---|
| 1 | `https://jenkins.example.com/` 접속 | admin 로그인 OK, JCasC `Last Applied` 성공 |
| 2 | Manage Nodes → `linux-agent-1` | Online (에이전트가 inbound 연결됨) |
| 3 | freestyle job: `echo hello && sleep 5` 실행 | 성공 |
| 4 | 컨트롤러 인스턴스 강제 종료: `aws ec2 terminate-instances --instance-ids <id>` | ASG가 신규 인스턴스 launch (~3분) |
| 5 | 신규 인스턴스 부팅 완료 후 같은 URL 재접속 | 잡 이력·credentials 그대로 (EBS 보존) |
| 6 | DLM 일정 시각 후 EBS 스냅샷 자동 생성 확인 | `aws ec2 describe-snapshots --owner-ids self --filters Name=tag:Name,Values=jenkins-data` |
